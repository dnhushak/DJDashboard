...
public class TestParser 
{
	...
	public void testUniqueGenres()
	{
		...
		//assertEquals("Genre should be null", null, library.get(0).verifyGenre(genres, "steve"));
		//assertEquals("Genre should be null", "rock", library.get(0).verifyGenre(genres, "rock"));
	...
	public void testContainsExplicitTag()
	{
		...
		for(Track track : library)
		{
			if(track.getName().toUpperCase().contains("[EXPLICIT]"))
			{
				count++;
			}
		}
	...
	public void testContainsRecoTag()
	{
		...
		for(Track track : library)
		{
			if(track.getName().toUpperCase().contains("[RECO]"))
			{
				count++;
			}
		}
	...
	public void testRemoveMultipleTagsOnOneTrack()
	{
		...
		track.setName("[Explicit] [RECO]The     track is 			This is a track name [RECO] [EXPLICIT]");
		assertEquals("Track name should contain no tags", "Track Is This Is A Track Name", track.getName());
	}
}
