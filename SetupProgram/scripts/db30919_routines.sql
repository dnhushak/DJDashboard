...
#First Check for exisiting distributor
	SET @DistributorID = (SELECT d.iddistributor FROM db30919.distributor d
		WHERE d.name = aDistributorName);
	IF(@DistributorID IS NOT NULL) THEN
		#If it exists, then updated it
		UPDATE db30919.distributor
		SET
	...
	ELSE
	#Else create new
		INSERT INTO db30919.distributor
			(name,
			phone,
			email,
			contactName,
...
BEGIN
	...
	ELSE 
		INSERT INTO db30919.usertype(typename, LibraryView, LibraryManage, PSAView, PSAEdit, GrantView, GrantEdit, ManageUsers, PlaylistEditor, EditPermissions, OnAirSignon, ReviewMusic)
